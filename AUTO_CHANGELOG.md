...
- 사용자 검토 화면을 기준으로 로그인과 회원가입의 버튼·링크 번호 위치를 다시 맞췄습니다.
- 로그 상세의 메모, 유사 로그 비교와 웨이트 비교 번호가 빈 영역으로 내려가던 문제를 수정했습니다.
- 내 정보의 다이빙 자격, 연동 기기, 정보 수정, 로그 시작 번호와 자격증 등록 번호 위치를 각 영역에 맞췄습니다.
- 화면 설계서 버전을 v1.7로 올리고 수정 대상 4페이지를 확대 렌더링해 위치를 확인했습니다.
...
- 화면 설계서의 13개 와이어프레임에 디스크립션 표의 `NO`와 대응하는 파란 번호를 추가했습니다.
- 화면별 디스크립션 행 수와 와이어프레임 번호 수가 모두 일치하는지 확인했습니다.
- 최종 확인 및 저장 화면에서 와이어프레임에 없는 `수정 버튼` 설명을 실제 표시된 `이전 버튼` 동작으로 수정했습니다.
- 화면 설계서 버전을 v1.6으로 올리고 PDF 14페이지 전체의 번호 위치와 표 연결을 검수했습니다.
...
- 구현 전 설계 단계에 맞춰 화면 설계서의 `화면 캡처` 표현을 `화면 와이어프레임`으로 변경했습니다.
- 이미지는 실제 구현 결과가 아니라 화면 구조와 정보 배치를 확인하기 위한 HTML 프로토타입 기반 설계 예시임을 명시했습니다.
- 화면 설계서 버전을 v1.5로 올리고 PDF 14페이지에서 기존 `화면 캡처` 표현이 남지 않았는지 확인했습니다.
...
- 요구사항 18개의 중요도와 난이도를 `상·중·하`로 평가해 요약표에 반영했습니다.
- 중요도는 핵심 흐름, 데이터 접근·저장과 선행 조건 영향을 기준으로 정했습니다.
- 난이도는 외부 연동, 계산, 권한, 파일 처리와 상태 관리 범위를 기준으로 정했습니다.
- 요구사항 정의서 버전을 v1.4로 올리고 PDF를 다시 생성해 9페이지 전체를 검수했습니다.
...
- 참고 제출 자료의 작성 항목을 반영해 요구사항 정의서에 대분류, 중분류, 중요도, 난이도와 제약사항을 한눈에 보는 요약표를 추가했습니다.
- 아직 결정되지 않은 중요도와 난이도는 임의로 평가하지 않고 `미정`으로 표시했습니다.
- 화면 설계서 PDF를 가로형 14페이지로 재구성해 13개 화면의 기본 정보, 실제 캡처, UI 설명과 예외 처리를 화면별 한 페이지에 배치했습니다.
- 두 PDF의 작성자를 김가은으로 설정하고 요구사항 정의서 v1.3, 화면 설계서 v1.4로 갱신했습니다.
...
```text
 GANI-Log-요구사항정의서.md              |   37 +-
 GANI-Log-화면설계서.md                  |   11 +-
 output/pdf/GANI_Log_요구사항_정의서.pdf |  Bin 134050 -> 139868 bytes
 output/pdf/GANI_Log_화면_설계서.pdf     | 1248 ++++++++++++++-----------------
 4 files changed, 600 insertions(+), 696 deletions(-)
```

</details>

<!-- change-id: fb268dfd81b80fae06ad632f0f7502dcfe86c009541d120f3f23352d5b0bf9c9 -->
...
```text
 GANI-Log-요구사항정의서.md              |  42 +++++++++++++++++---------------
 output/pdf/GANI_Log_요구사항_정의서.pdf | Bin 139868 -> 142270 bytes
 2 files changed, 22 insertions(+), 20 deletions(-)
```

</details>

<!-- change-id: 75000e4019758be08c33b68585449281ccbde406a7d85a840da721b4835c9b73 -->
...
```text
 GANI-Log-화면설계서.md              |  34 +-
 output/pdf/GANI_Log_화면_설계서.pdf | 912 +++++++++++++++++++-----------------
 2 files changed, 493 insertions(+), 453 deletions(-)
```

</details>

<!-- change-id: 7c6ba435c81e82ed7df17ece49ffc12f43423b580a325179e5a8941834e43c2f -->
...
```text
 GANI-Log-화면설계서.md              |   6 +-
 output/pdf/GANI_Log_화면_설계서.pdf | 850 +++++++++++++++++-------------------
 2 files changed, 413 insertions(+), 443 deletions(-)
```

</details>

<!-- change-id: 3bb3da11427bbaa642d72524552071e05817556491e5d30c5d92362adfc1874a -->
...
```text
 GANI-Log-화면설계서.md              |  3 +-
 output/pdf/GANI_Log_화면_설계서.pdf | 78 ++++++++++++++++++-------------------
...
```text
A	"\354\232\270\354\202\2603\353\260\230_MiniProject_\352\271\200\352\260\200\354\235\200.zip"
A	"\354\232\270\354\202\2603\353\260\230_MiniProject_\352\271\200\352\260\200\354\235\200/API.pdf"
A	"\354\232\270\354\202\2603\353\260\230_MiniProject_\352\271\200\352\260\200\354\235\200/API.yaml"
A	"\354\232\270\354\202\2603\353\260\230_MiniProject_\352\271\200\352\260\200\354\235\200/DB ERD.dbml"
A	"\354\232\270\354\202\2603\353\260\230_MiniProject_\352\271\200\352\260\200\354\235\200/ERD.pdf"
A	"\354\232\270\354\202\2603\353\260\230_MiniProject_\352\271\200\352\260\200\354\235\200/\354\232\224\352\265\254\354\202\254\355\225\255 \354\240\225\354\235\230\354\204\234.pdf"
A	"\354\232\270\354\202\2603\353\260\230_MiniProject_\352\271\200\352\260\200\354\235\200/\355\231\224\353\251\264 \354\204\244\352\263\204\354\204\234.pdf"

 ...oject_\352\271\200\352\260\200\354\235\200.zip" |   Bin 0 -> 1774945 bytes
 .../API.pdf"                                       |   255 +
 .../API.yaml"                                      |  1737 ++
 .../DB ERD.dbml"                                   |   380 +
 .../ERD.pdf"                                       | 30949 +++++++++++++++++++
 ...5\255 \354\240\225\354\235\230\354\204\234.pdf" |   Bin 0 -> 142270 bytes
 ...1\264 \354\204\244\352\263\204\354\204\234.pdf" |   Bin 0 -> 1441144 bytes
 7 files changed, 33321 insertions(+)
```
...
- 7개 파일의 변경 내용을 저장했습니다.

### 변경된 파일

- 추가: `울산3반_MiniProject_김가은.zip`
- 추가: `울산3반_MiniProject_김가은/API.pdf`
- 추가: `울산3반_MiniProject_김가은/API.yaml`
- 추가: `울산3반_MiniProject_김가은/DB ERD.dbml`
- 추가: `울산3반_MiniProject_김가은/ERD.pdf`
- 추가: `울산3반_MiniProject_김가은/요구사항 정의서.pdf`
- 추가: `울산3반_MiniProject_김가은/화면 설계서.pdf`

<details>
<summary>파일별 변경 규모 보기</summary>

```text
 울산3반_MiniProject_김가은.zip                 |   Bin 0 -> 1774945 bytes
 울산3반_MiniProject_김가은/API.pdf             |   255 +
 울산3반_MiniProject_김가은/API.yaml            |  1737 ++
 울산3반_MiniProject_김가은/DB ERD.dbml         |   380 +
 울산3반_MiniProject_김가은/ERD.pdf             | 30949 +++++++++++++++++++++++
 울산3반_MiniProject_김가은/요구사항 정의서.pdf |   Bin 0 -> 142270 bytes
 울산3반_MiniProject_김가은/화면 설계서.pdf     |   Bin 0 -> 1441144 bytes
 7 files changed, 33321 insertions(+)
```

</details>

<!-- change-id: c94104c724824809dd49ec2d30d54cb0696bec09fe6e8e334816f623d6e284be -->
